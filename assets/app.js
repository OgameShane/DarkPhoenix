async function loadData() {
    ...
    for (var i = 0; i < playersDOM.length; i++) {
        if (playersDOM[i].hasAttribute('alliance') && parseInt(playersDOM[i].getAttribute('alliance')) === 500020) {
            players.push({
...
new Vue({
    ...
    created() {
        var parent = this;
        loadData().then(function(players) {
            var map = [];
            ...
            // Freezing this data hugely improves Vues abaility to iterate over all the elements in the system view
            Object.freeze(parent.map);
            Object.freeze(parent.players);
            Object.freeze(parent.galaxyViewData);
            parent.loaded = true;
        });
    },
    template: `
